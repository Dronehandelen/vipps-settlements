...
const subscriberDirectory = './src/crons';
const directoryPath = path.join(__dirname, subscriberDirectory);
const kubernetesTemplate = path.join(__dirname, 'k8s/cronTemplate.yaml');

const outputYamlPath = path.join(
    __dirname,
    'chart/templates/generated_crons.yaml'
);
...
fs.readdir(directoryPath, function (err, files) {
    ...
    Promise.all(files.map((file) => createDeployment(file))).then(
        (jobDescriptions) => {
            ...
            fs.writeFileSync(outputYamlPath, jobsFileContent);

            console.log('summary: ');
            console.log(
                jobDescriptions
                    .filter((x) => x)
                    .map((j) => `${j.scriptName} - ${j.schedule}`)
            );
            process.exit(0);
        }
    );
